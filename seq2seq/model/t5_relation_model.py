...
class T5Attention(nn.Module):
    # There is a Relational-Attention-Transformer Model
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        relative_buckets = 0
        if bidirectional:
            num_buckets //= 2   # This same as "num_buckets = num_buckets // 2" , "//" is 'zheng chu'
    ...
    def forward(
        ...
        if relation_k_emb is None:
            # compute scores  Q x K  (no RAT (Relational-Attention-Transformer))
            scores = torch.matmul(
                query_states, key_states.transpose(3, 2)
            )  # equivalent of torch.einsum("bnqd,bnkd->bnqk", query_states, key_states), compatible with onnx op>9
        ...
        #=============================================以下计算关系部分=====================================================================
            ...
            # 2. 接下来计算关系矩阵的积  , compute of RAT about Q_State & K_state
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if relations is not None and self.relation_k_emb is not None:
            ...
            # print("Yeah !!! we Put relation into the encoder model !!!")
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, self.relation_k_emb, self.relation_v_emb) # ***Note: relation_k_emb
    ...
    def forward(
        ...
        # lm_logits = self.lm_head(sequence_output) # ** original code

        lm_logits = self.lm_head(hidden_states)   # ** We do not need Decoder
