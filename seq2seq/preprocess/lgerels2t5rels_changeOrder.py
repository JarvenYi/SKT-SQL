...
def match_question(t5_processed, dataset_lgesql, t5_tokenizer, lge_tokenizer, dataset_name,  mode):
    ...
    for lge_dataset_idx in tqdm(range(len(dataset_lgesql))):
        lge_aux_question_idx_list = get_idx_list_changeOrder(dataset_lgesql[lge_dataset_idx], dataset_name)     # if single-turn, return -1
            ...
            for k, question_idx in enumerate(lge_aux_question_idx):
                ...
                if k == 0:
                    ...
                    try:
                        ...
                        if (len(t5_toks)<512):
                            err += 1
                            print("ERROR reason is: len(t5_tok) < 512")
                        continue
...
def generate_relations(dataset_lgesql, t5_processed, table_lgesql, RELATION2ID_DICT, edgeType, t5_tokenizer, dataset_name, coref_dataset, use_dependency, mode):
    ...
    for lge_dataset_idx in tqdm(range(len(dataset_lgesql))):
        lge_aux_question_idx_list = dataset_lgesql[lge_dataset_idx]["idx_list"]
        for j, lge_aux_question_idx in enumerate(lge_aux_question_idx_list):
            '''t5_toks_id[2]: the ids of tokens of j-th question in the sample.
                t5_toks_id[1]: the tokens of j-th question in the sample.
                t5_toks_id[0]: the ids of the sample    - Jarven'''
            t5_toks_ids = dataset_lgesql[lge_dataset_idx][f"t5_toks_{j}"][2]
            ...
            # Get "question-question-generic" and "question-question-dist{distance}" into relation
...
def init_tokenizer():
    t5_tokenizer = AutoTokenizer.from_pretrained('model_download_by_Jarven/t5-small') # t5-small -> model_download_by_Jarven/t5-small
    lge_tokenizer = AutoTokenizer.from_pretrained('model_download_by_Jarven/t5-small') # t5-small -> model_download_by_Jarven/t5-small
