...
def parse_option():
    ...
    parser.add_argument('--input_dataset_path', type = str, default = "./data/preprocessed_data/preprocessed_train_spider.json",
                        help = 'filepath of the input dataset.')
    parser.add_argument('--output_dataset_path', type = str, default = "./data/preprocessed_data/sktsql_train.json",
...
def prepare_input_and_output(opt, ranked_data):
    ...
    # *** 2024-3-16 Add Hardness CLS
    if opt.mode == 'train':
        # if ranked_data["hardness_labels"] == 0:      Hds = "[/easy] "
        # elif ranked_data["hardness_labels"] == 1:    Hds = "[/medium] "
        # elif ranked_data["hardness_labels"] == 2:    Hds = "[/hard] "
        # elif ranked_data["hardness_labels"] == 3:    Hds = "[/extra] "
        if opt.target_type == "sql":
            skeletons = ranked_data["sql_skeleton"]
            # input_sequence = question + schema_sequence
            # input_sequence = skeletons + " | " + question + schema_sequence
            input_sequence = "<START> " + skeletons + " " + question + schema_sequence
        elif opt.target_type == "natsql":
            natsql_skeletons = ranked_data["natsql_skeleton"]
            input_sequence = "<START> " + natsql_skeletons + " " + question + schema_sequence
    else:
        input_sequence = question + schema_sequence
        
    if opt.target_type == "sql":
        output_sequence = ranked_data["norm_sql"]
        output_skeleton = ranked_data["sql_skeleton"]
    elif opt.target_type == "natsql":
        output_sequence = ranked_data["norm_natsql"]
        output_skeleton = ranked_data["natsql_skeleton"]

    return input_sequence, output_sequence, output_skeleton
...
def generate_train_ranked_dataset(opt):
    ...
    for data_id, data in enumerate(dataset):
        ...
        # ------------Hardness -BY Jarven------------
        ranked_data["hardness"] = data["hardness"]
        ranked_data["hardness_labels"] = data["hardness_labels"]
        # -------------------------------------------
        ...
        if random.random() < opt.noise_rate:
            random.shuffle(topk_table_ids)  # shuffle sequential order of the top-k table-ids as the noise
        ...
        input_sequence, output_sequence, output_skeleton = prepare_input_and_output(opt, ranked_data)
        
        # record table_name_original.column_name_original for subsequent correction function during inference
        tc_original = []
        table_srf, column_srf = [], []
        ...
        output_dataset.append(
            {
                ...
                "hardness_labels": ranked_data["hardness_labels"]
            }
        )
...
if __name__ == "__main__":
    ...
    if opt.mode == "train":
        ...
        print("Finished Train RESD data")
    elif opt.mode in ["eval", "test"]:
        generate_eval_ranked_dataset(opt)
        print("Finished Eval RESD data")
    else:
