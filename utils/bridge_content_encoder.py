...
def get_matched_entries(
    ...
    for field_value in field_values:
        ...
        sm = difflib.SequenceMatcher(None, n_grams, fv_tokens)  # ** 比较文本内容，返回差异部分
        match = sm.find_longest_match(0, len(n_grams), 0, len(fv_tokens)) # ** 查找最长的匹配块 返回一个命名元组 Match(a, b, size)
        if match.size > 0: # Exit Match part
...
def get_column_picklist(table_name: str, column_name: str, db_path: str) -> list:
    ...
    try: # ** verify table_name_original & column_name_original is legal in SQL DB
        conn = sqlite3.connect(db_path)  # ** sqlite3.connect() can create or connect a DataBase
        conn.text_factory = bytes
        c = conn.cursor()       # ** create DataBase object
        c.execute(fetch_sql)    # ** execute SQL operate
        picklist = set()
        for x in c.fetchall():  # ** c.fetchall() is the return collect of c.execute()
...
def get_database_matches(
    ...
    picklist = get_column_picklist(     # ** verify table_name_original & column_name_original is legal in SQL DB
